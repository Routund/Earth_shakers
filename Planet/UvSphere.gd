...
func initiate_impact(impact_site : Vector3, strength : float):
	add_impact.rpc(impact_site,strength)
...
func add_impact(impact_site : Vector3, strength : float):
	...
	for i in range(len(impact_points)):
		impact_image.set_pixel(i,0,Color(impact_points[i][0].x/2 + 0.5,impact_points[i][0].y/2 + 0.5,impact_points[i][0].z/2 + 0.5,impact_points[i][1]/900))
...
func _process(delta : float) -> void:
	material_override.set("shader_parameter/outside_time", k)
	k+=delta  
	Gravity.k = k
